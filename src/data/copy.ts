export const APP_COPY = {
	...
	economyTemplate: (times: number, saved: string) => `Se você tivesse feito ${times} a menos por semana, teria sobrado ${saved} no bolso.`,

	archetypes: {
		acumulador: {
			name: 'O Acumulador',
			icon: '🐿️',
			description: 'Compra tudo que vê, mas sempre acha que é investimento. Seu quarto é quase um estoque.',
		},
		gastadora_impulsiva: {
	...
	weeklyBehavior: {
		inicio_gastoalto: 'Você gastou no começo como quem disse: ‘depois eu vejo’ (spoiler: depois chega).',
		final_gastoalto: 'O gasto guardou energia pro final e soltou tudo na reta final. Um ‘modo cautela’ nos últimos dias já faz diferença.',
		constante: 'Você gastou bem distribuído: sem sustos, sem picos. Isso é ótimo pra manter o controle.'
	}
}